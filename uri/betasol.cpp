...
/** EVERYTHING BELOW THIS POINT HAS NOT BEEN TESTED YET **/

/**
 * Adds a state to a set of next states to visit
 * if the state has not previously been visited.
 * Adds the state to the visited set if it has just been discovered.
 */
void addToNextStates(unordered_set<int32_t> &nextStates, int32_t key) {
	if(visitedStates.count(key) == 0) {
		addState(nextStates, key);
		addState(visitedStates, key);
	}
}
...
unordered_set<int32_t> transition(int32_t key) {
	...
	if(replaceIndex.empty() || replaceIndex.size() == 3) {
		...
		addToNextStates(nextStates, stateToKey(s));
	} else {
		...
		if(replaceIndex.size() == 1) {
			for(int i = 0; i < a.size(); i++) {
				...
				addToNextStates(nextStates, stateToKey(newS));
			}
